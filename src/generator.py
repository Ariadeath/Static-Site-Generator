...
def generate_page(from_path, template_content, dest_path):
...
def generate_pages_recursive(dir_path_content, template_content, dest_dir_path):
    ...
    for item in dir_path_content.iterdir():
        if item.is_file() and item.suffix == '.md':
            ...
            generate_page(str(item), template_content, str(dest_path))
        elif item.is_dir():
            new_dest_dir = dest_dir_path / item.name
            new_dest_dir.mkdir(parents=True, exist_ok=True)

            generate_pages_recursive(item, template_content, new_dest_dir)